...
class PwUbbCode
{
    ...
    {
        $message = preg_replace_callback("/\[url=((https?|ftp|gopher|news|telnet|mms|rtsp|thunder|ed2k)?[^\[\s]+?)(\,(1)\/?)?\](.+?)\[\/url\]/is", function ($m) use ($checkurl) {
            return PwUbbCode::createUrl($m[1], $m[5], $m[2], $m[4], $checkurl);
        }, $message);
        $message = preg_replace_callback("/\[url\]((https?|ftp|gopher|news|telnet|mms|rtsp|thunder|ed2k)?[^\[\s]+?)\[\/url\]/is", function ($m) use ($checkurl) {
    ...
    {
        return preg_replace_callback("/\[code(\s*brush\:(.+?)\;toolbar\:(true|false)\;)?\](.+?)\[\/code\]/is", function ($m) {
    ...
    {
        return preg_replace_callback("/\[sell=(\d+)(\,(\d+))?\](.+?)\[\/sell\]/is", function ($m) use ($config) {
    ...
    {
        if ($convertStatus) {
            return preg_replace_callback("/\[(wmv|mp3)(=(0|1))?\]([^\<\(\r\n\"']+?)\[\/\\1\]/is", function ($m) {
                return PwUbbCode::createPlayer($m[4], '314', '53', $m[3], 'audio');
            }, $message, self::$_cvtimes);
            return preg_replace_callback("/\[(wmv|rm)(=([0-9]{1,3})\,([0-9]{1,3})\,(0|1))?\]([^\<\(\r\n\"']+?)\[\/\\1\]/is", function ($m) {
    ...
    {
        return preg_replace_callback("/\[iframe\]([^\[\<\(\r\n\"']+?)\[\/iframe\]/is", function ($m) use ($convertStatus) {
    ...
    public static function autoUrl($message, $hasCode = false)
    {
        if ($hasCode) {
            self::_init();
            self::_startParse();
            self::hasTag($message, 'code') && $message = preg_replace_callback("/\[code.*?\].+?\[\/code\]/is", function ($m) {
    ...
    public static function createTable($text, $width = '', $bgColor = '', $borderColor = '', $borderWidth = '', $cellpadding = '', $align = '')
    {
        ...
        $text = preg_replace_callback("/\[(tr|\/td)\]\s*\[td(=(\d{1,2}),(\d{1,2})(,(\d{1,3}(\.\d{1,2})?(%|px)?))?)?\]/is", function ($m) use ($tdStyle) {
            return PwUbbCode::createTd($m[1], $m[3], $m[4], $m[6], $tdStyle);
        }, $text);
